		...
		switch(f1.bus_type.value)
		{
			case '1'://立即分账，通用
			{
				...
				f1.bus_args.value = iRefundTotleFee.toString()
													 +"|7457222^"+iProviderRefundFee.toString()
			...
			case '2'://立即分账，51666用
			{				
				f1.bus_args.value = iRefundCnt.toString()+"|7457222|"+iRefundTotleFee.toString();
				break;
			}
			case '33'://延迟分账，通用
			{
...
<?php
	...
	if(!isset($frm_spid)) $frm_spid="2000000501"; 
	if(!isset($frm_fee)) $frm_fee="50800";
	if(!isset($frm_ver)) $frm_ver="4";
	if(!isset($frm_tid)) $frm_tid="2000000501200807070000000001";
	...
	if(!isset($frm_refundid)) $frm_refundid=sprintf("109".$frm_spid . "%07d",time()%10000000);;
?>
<form id="frm" action="/Pay/split_refund_b2c_1.php">
	...
	<fieldset ><legend>退款信息</legend>
		<label>退款总额</label>必须全额退款<br>
